...
uniform float freq_x = 1.0, freq_y = 1.0;
...
float sample_noise(vec2 texture_coords, vec4 offset, vec2 freq)
{
  ...
  float n;
  if(type == 1)
    n = noise(vec2(offset.x + freq.x * texture_coords.x, offset.y + freq.y * texture_coords.y));
  else if(type == 2)
    n = snoise(vec2(offset.x + freq.x * texture_coords.x, offset.y + freq.y * texture_coords.y));
  else if(type == 3)
    n = noise(vec3(offset.x + freq.x * texture_coords.x, offset.y + freq.y * texture_coords.y, offset.z));
  else if(type == 4)
    n = snoise(vec3(offset.x + freq.x * texture_coords.x, offset.y + freq.y * texture_coords.y, offset.z));
  else if(type == 5)
    n = noise(vec4(offset.x + freq.x * texture_coords.x, offset.y + freq.y * texture_coords.y, offset.z, offset.w));
  else if(type == 6)
    n = snoise(vec4(offset.x + freq.x * texture_coords.x, offset.y + freq.y * texture_coords.y, offset.z, offset.w));
  else
    n = 0;
...
{
  vec2 tex = vec2(texture_coords.x, 0.0);
  vec4 offset = vec4(x, y, z, w);
  vec2 freq = vec2(freq_x, freq_y);
  float n = sample_noise(tex, offset, freq);

  freq.x *= 5;
  offset.x *= 5;

  n += .2 * sample_noise(tex, offset, freq);

  freq.x *= 5;
  offset.x *= 5;

  n += .02 * sample_noise(tex, offset, freq);

  // Bring noise into [0.25, 0.75]
  n = 0.5 + 0.25 * n;
